...
const getTempLeaderBoard = async () => {
  try {
    // Axis API GET call
...
const sortArrayByMmr = scholars => {
  try {
    ...
    let tableLeader = []
    sortedScholars.forEach( element => {
      tableLeader.push( [element.manager, element.name, element.mmr])
    });
    console.table(tableLeader);
...
module.exports = getTempLeaderBoard;